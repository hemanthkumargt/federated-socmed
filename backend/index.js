...
io.on("connection", (socket) => {
  ...
  // Typing indicator: forward "typing" status to the recipient
  socket.on("userTyping", ({ fromUserId, toUserId }) => {
    const recipientSocketId = onlineUsers.get(toUserId)
    if (recipientSocketId) {
      io.to(recipientSocketId).emit("userTyping", { fromUserId })
    }
  })

  // Stop typing: forward "stop typing" status to the recipient
