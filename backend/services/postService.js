 ...
 * Increments repostCount on the original post.
 */
export const repostPostService = async ({
    originalPost,
    repostAuthorFederatedId,
    repostAuthorDisplayName,
    repostFederatedId,
    originServer
}) => {
    // Increment original post's repost count
    await Post.findByIdAndUpdate(originalPost._id, { $inc: { repostCount: 1 } });

    // Chain to the root of the repost tree
    const trueOriginalId = originalPost.isRepost
        ? originalPost.originalPostFederatedId
        : originalPost.federatedId;

    const trueOriginalAuthorFedId = originalPost.isRepost
        ? originalPost.originalAuthorFederatedId
        : originalPost.authorFederatedId;

    const trueOriginalAuthorName = originalPost.isRepost
        ? originalPost.originalAuthorDisplayName
        : originalPost.userDisplayName;
    ...
    const repost = new Post({
        ...
        isUserPost: true,
        userDisplayName: repostAuthorDisplayName,
        authorFederatedId: repostAuthorFederatedId,
        isChannelPost: false,
        channelName: null,

        isRepost: true,
        originalPostFederatedId: trueOriginalId,
        originalAuthorFederatedId: trueOriginalAuthorFedId,
        originalAuthorDisplayName: trueOriginalAuthorName,

        federatedId: repostFederatedId,
        originServer,
        serverName: originServer,
        isRemote: false,
        federationStatus: 'local',
