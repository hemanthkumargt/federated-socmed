...
export const repostPost = async (req, res, next) => {
  try {
    ...
    const originalPost = await Post.findOne({ federatedId: postFederatedId });

    if (!originalPost) {
      return next(createError(404, 'Original post not found'));
    }

    if (originalPost.isRemote) {
      return next(createError(403, 'Cannot repost remote content directly'));
    }

    // Prevent double-reposting the same post
    const alreadyReposted = await Post.findOne({
      authorFederatedId: req.user.federatedId,
      originalPostFederatedId: postFederatedId,
      isRepost: true
    });

    if (alreadyReposted) {
      return next(createError(409, 'You have already reposted this post'));
    }

    const repostFederatedId = `${req.user.federatedId}/repost/${Date.now()}`;

    const repost = await repostPostService({
      originalPost,
      repostAuthorFederatedId: req.user.federatedId,
      repostAuthorDisplayName: req.user.displayName,
      repostFederatedId,
      originServer: req.user.serverName
    });

    // Optionally federate the repost
    if (originalPost.originServer !== process.env.SERVER_NAME) {
      await sendFederationEvent({
        type: 'REPOST',
        actorFederatedId: req.user.federatedId,
        objectFederatedId: postFederatedId
      }).catch(err => console.error('Repost federation error:', err));
    }
